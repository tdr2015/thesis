...
##############################################################
    ...
    # 'Or' is shown by | 'and' is shown by &       
...
#    prop_impact.to_csv(Property_outfile_one) 
    ...
    fld_pt_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_one,2), 'OP4_AAPE'])
    ppl_pt_one = fld_pt_one_lookup*(1+a_ppl)
    NO_PPL_PT_ONE.append(ppl_pt_one)
       
    fld_pt_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_two,2), 'OP4_AAPE'])
    ppl_pt_two = fld_pt_two_lookup*(1+a_ppl)
    NO_PPL_PT_TWO.append(ppl_pt_two)

    fld_pt_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_five,2), 'OP4_AAPE'])
    ppl_pt_five = fld_pt_five_lookup*(1+a_ppl)
    NO_PPL_PT_FIVE.append(ppl_pt_five)    
    
    fld_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_one,2), 'OP4_AAPE'])
    ppl_one = fld_one_lookup*(1+a_ppl)
    NO_PPL_ONE.append(ppl_one)    
    
    fld_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_two,2), 'OP4_AAPE'])
    ppl_two = fld_two_lookup*(1+a_ppl)
    NO_PPL_TWO.append(ppl_two)    
    
    fld_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_five,2), 'OP4_AAPE'])
    ppl_five = fld_five_lookup*(1+a_ppl)
    NO_PPL_FIVE.append(ppl_five)     
    
    fld_ten_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_ten,2), 'OP4_AAPE'])
    ppl_ten = fld_ten_lookup*(1+a_ppl)
    NO_PPL_TEN.append(ppl_ten)     
    
    fld_twenty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_twenty,2), 'OP4_AAPE'])
    ppl_twenty = fld_twenty_lookup*(1+a_ppl)
    NO_PPL_TWENTY.append(ppl_twenty) 
       
    fld_forty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_forty,2), 'OP4_AAPE'])
    ppl_forty = fld_forty_lookup*(1+a_ppl)
    NO_PPL_FORTY.append(ppl_forty)

    fld_sixty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_sixty,2), 'OP4_AAPE'])
    ppl_sixty = fld_sixty_lookup*(1+a_ppl)
    NO_PPL_SIXTY.append(ppl_sixty)

    fld_ninety_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_ninety,2), 'OP4_AAPE'])
...
#Property_outfile_one = "C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Prop_Impact_op4.csv"
#prop_impact.to_csv(Property_outfile_one) 

Cases_outfile_one = ("C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Case_Results_RDM_OPTION4_RETREAT.csv")
