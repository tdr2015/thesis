...
CaseStudyTwo_DAPP.py -   The script estimates the timing at which average 
                         annual number of people exposed to flooding and the
                         average annual monetary impacts to private dwellings
                         is reached. It uses transient scenarios (i.e. time 
                         dependent realisations into the future) to assess the
                         time at which adaptaiton tipping points are reached
                         (also called use0by years).
                         
                         The script is the second of two scripts 
                         (CaseStudy_RDM.py). The script also uses a lookup 
                         table produced in CaseTwo_AAPE_Lookup.py
...
#cases_dapp = pd.read_csv('C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\2018-03-07_Case_Results_RDM.csv')
cases_dapp.head()

## Define prior values for key factors
cases_dapp = cases_dapp[0:1]    #Note [31:32] a good one to test
cases_dapp
...
#    timeframe = int(t_lead_pol_a + p_life_pol_a)
    timeframe=90
#    year = range(0,timeframe +1)
    year = range(0,timeframe +1,5) ## 5-yearly    
    
    columns = []                # Set up emply 
#    index = range(0,timeframe + 1)
    index = range(0,timeframe + 1,5) ## 5-yearly   
...
#        prop_impact['Str_Value'] = prop_impact['FLR_AREA'] * prop_impact['STOREYS'] * j['rate_str']
...
#    ax3.set_xticklabels([])
    ...
    plt.axhline(y=BL_AAD_risk, xmin=0, xmax=1, linewidth=2, color = 'k', linestyle = '-.')
    plt.text(2020, BL_AAD_risk*3, "ATP = $" + str(BL_AAD_risk) + " /yr")
    plt.ylabel('AAD ($/yr)',fontsize = 8)
    plt.yticks(np.arange(0,100000001,25000000))
#    plt.xlim(2010,2120)
#    plt.xlabel('Time (year)')
    ax3.set_xticklabels([])
    ax4 = plt.subplot(414)
    plt.plot(annual_MSL['Year_Date'],annual_MSL['AAPE'])     #PROB NEED TO USE PV_AAD FOR VULN?
    plt.axhline(y=BL_AAPE_risk, xmin=0, xmax=1, linewidth=2, color = 'k', linestyle = '-.')
...
#annual_MSL_outfile_two = "C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_annual_msl.csv"
...
fig.savefig('C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\'+date_string+'_scenarios'+'.png',dpi=300,bbox_inches='tight')
plt.show()
...
plt.ylabel('Year')
plt.text(1.1, sign_median+1, "Median = " + str(sign_median), fontsize = 7)
plt.text(1.1, sign_max -3, "Max = " + str(sign_max), fontsize = 7)
plt.text(1.1, sign_min, "Min = " + str(sign_min), fontsize = 7)
plt.text(1.1, np.percentile(cases_dapp["ATP_AAD"],25)-1, "Q1 = " + str(int(np.percentile(cases_dapp["ATP_AAD"],25))), fontsize = 7)
...
plt.text(1.1, sign_max1 -3, "Max = " + str(sign_max1), fontsize = 7)
...
fig2.savefig('C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\'+date_string+'_boxplot'+'.png',dpi=300)
plt.show()
...
Cases_outfile_two = "C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Case_Results_DAPP.csv"
