...
#    agg_ppl = 0    
    ...
    ##############################################################     
    ##############################################################
        # DON'T SAMPLE HOUSES IN ZONE 1, 2, 3, 4
        ...
        prop_imp_samp = prop_imp_sub.loc[prop_imp_sub['sample']=='N']  ##Select remianing propoerties that have not been sampled

        if j['Year_Date'] <= year_imp:    #i.e. option does not commence until 2030     
#        if 2035 < year_imp:    #i.e. option does not commence until 2030     
            prop_impact = prop_impact              
        else:
            nn = rate_op # Average number of houses retreating per 5 years
    
            if len(prop_imp_sub.index) < nn: #Rate of properties retreated
                nn = len(prop_imp_sub.index)
            else:
                nn = nn
        
            #Randomly sample 4 properties
            if nn > 0:
                prop_imp_samp2 = pd.DataFrame(prop_imp_samp.sample(nn))  #Sample from remaining properties that have not yet been sampled
                
                FLRTWO = []
                VM = []
#                PPL_New=[]
                for index, x in prop_imp_samp2.iterrows():      
                    EST_FLR = x['EST_FLR_LVL_R']     
                    VULN = x['VULN_MODEL']
                    ppl_new_exp = x['AV_PPL_DWL']
                    if EST_FLR < 2.6:   #MIN FLOOD LEVEL 2.6m
                        FLR2 = 2.6
                    else:
                        FLR2 = EST_FLR
                    FLRTWO.append(FLR2)
            
                    if x['BLD_TYPE'] == 'Residential':
                        VM2 = 'FCM1'
                    else:
                        VM2 = VULN  
                    VM.append(VM2)
#                    PPL_New.append(ppl_new_exp)
                
                prop_imp_samp2['FloorTwo'] = FLRTWO
                prop_imp_samp2['VulnModel'] = VM     
#                prop_imp_samp2['PPL_risk'] = PPL_New
                
                prop_imp_samp2.head()
                # Replace the value in the original dataset 
                for index, y in prop_imp_samp2.iterrows():          
                    ID = int(y['Arc_ID'])            
                    NewLVL = prop_imp_samp2.loc[prop_imp_samp2['Arc_ID'] == ID, 'FloorTwo']
                    NewVM = prop_imp_samp2.loc[prop_imp_samp2['Arc_ID'] == ID, 'VulnModel']
#                    Newppl = prop_imp_samp2.loc[prop_imp_samp2['Arc_ID'] == ID, 'PPL_risk']       
                    prop_impact.loc[prop_impact['Arc_ID'] == ID, 'EST_FLR_LVL_R'] = NewLVL
                    prop_impact.loc[prop_impact['Arc_ID'] == ID, 'VULN_MODEL'] = NewVM    
#                    prop_impact.loc[prop_impact['Arc_ID'] == ID, 'AV_PPL_DWL'] = Newppl
                    prop_impact.loc[prop_impact['Arc_ID']== ID, 'sample'] = 'Y'

#                agg_ppl = agg_ppl + sum(prop_imp_samp2['AV_PPL_DWL'])           
            
            else:
                prop_impact = prop_impact            
            
        prop_imp_retro = prop_impact.loc[prop_impact['sample'] == 'Y'] #Create a subset of those properties sampled (i.e. retrofitted) for use in AAPE calculations below
...
#            NO_PPL_NINETY.append(People)
        ...
        fld_pt_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_pt_one'],2), 'NO_OP_AAPE']) #baseline number of peopel exposed        
        ...
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner') #Assume sampled properties which have been 'improved' (floor level; infill) are not exposed. Identify which houses (ID) have been improved
        ppl_pt_one = (fld_pt_one_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl) #Baseline count of people less the sum of people in improved properties
        ppl_pt_one = np.clip(ppl_pt_one, 0, None)
        NO_PPL_PT_ONE.append(ppl_pt_one)

        fld_pt_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_pt_two'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_pt_two'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')
        ppl_pt_two = (fld_pt_two_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_pt_two = np.clip(ppl_pt_two, 0, None)
        NO_PPL_PT_TWO.append(ppl_pt_two)
    
        fld_pt_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_pt_five'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_pt_five'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_pt_five = (fld_pt_five_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_pt_five = np.clip(ppl_pt_five, 0, None)        
        NO_PPL_PT_FIVE.append(ppl_pt_five)    
        
        fld_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_one'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_one'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_one = (fld_one_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_one = np.clip(ppl_one, 0, None)        
        NO_PPL_ONE.append(ppl_one)    
        
        fld_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_two'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_two'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_two = (fld_two_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_two = np.clip(ppl_two, 0, None)         
        NO_PPL_TWO.append(ppl_two)    
        
        fld_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_five'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_five'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_five = (fld_five_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_five = np.clip(ppl_five, 0, None)         
        NO_PPL_FIVE.append(ppl_five)     
        
        fld_ten_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_ten'],2), 'NO_OP_AAPE'])        
        slr_name = int(100*round(j['aep_ten'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_ten = (fld_ten_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_ten = np.clip(ppl_ten, 0, None)         
        NO_PPL_TEN.append(ppl_ten)     
        
        fld_twenty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_twenty'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_twenty'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_twenty = (fld_twenty_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_twenty = np.clip(ppl_twenty, 0, None)         
        NO_PPL_TWENTY.append(ppl_twenty) 
    
        fld_forty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_forty'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_forty'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_forty = (fld_forty_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_forty = np.clip(ppl_forty, 0, None)         
        NO_PPL_FORTY.append(ppl_forty)
    
        fld_sixty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_sixty'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_sixty'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_sixty = (fld_sixty_lookup - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
        ppl_sixty = np.clip(ppl_sixty, 0, None)         
        NO_PPL_SIXTY.append(ppl_sixty)
    
        fld_ninety_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(j['aep_ninety'],2), 'NO_OP_AAPE'])
        slr_name = int(100*round(j['aep_ninety'],2))        
        Comparing_list = pd.read_csv("C:\\Users\\tdramm\\Desktop\\GIS_LE\\AAPE_Lookup\\Log\\"+str(slr_name)+"_houses_no_pol.csv")
        Merging = pd.merge(prop_imp_retro, Comparing_list, on=['Arc_ID'], how='inner')  
        ppl_ninety = (fld_ninety_lookup  - sum(Merging['AV_PPL_DWL']))*(1+r_ppl*year_ppl)
...
#Property_outfile_one = "C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Prop_Impact_op2.csv"
#prop_impact.to_csv(Property_outfile_one) 
#
## NOTE THIS IS THE FILE IN THE LAST CASE SCENARIO
#annual_MSL_outfile_two = "C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_annual_msl_op2.csv"
...
fig.savefig('C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\'+date_string+'_scenarios_op2'+'.png',dpi=300,bbox_inches='tight')
plt.show()
...
plt.xticks([1],["Option 2: New building regulations"])
...
#plt.ylabel('Year')
plt.text(1.1, sign_median1, "Median = " + str(sign_median1),fontsize = 7)
...
plt.text(1.1, np.percentile(cases_dapp["ATP_AAPE"],75), "Q3 = " + str(int(np.percentile(cases_dapp["ATP_AAPE"],75))), fontsize = 7)
plt.title('(b) AAPE')
...
fig2.savefig('C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\'+date_string+'_boxplot_op2'+'.png',dpi=300)
plt.show()
...
Cases_outfile_two = "C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Case_Results_DAPP_OPTION2.csv"
