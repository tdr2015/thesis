...
CaseStudyTwo_RDM.py -   The script estimates the average annual number of people 
                        exposed to flooding and the average annual monetary
                        impacts to private dwellings for different coastal 
                        inundation events due to changing sea levels in Lakes 
                        Entrance, Victoria. Impacts are determined without and with 
                        adaptation policy options. Different scenarios reflect 
                        combinations of uncertain factors. The inundation events 
                        considered include (1) permanent sea-level rise (i.e. loss) 
                        and (2) temporary flooding from extreme lake levels. The 
                        results of the analysis are exported in a .csv file. 
                        Analysis using Scenario Discovery (i.e. Robust 
                        decision making) can then be undertaken in R.                  
                                        
                        The script calls upon the ArcPy module to apply 
                        geoprocessing tools for undertaking spatial analysis. Open
                        source data from Geosceinces Australia and the Victorian
                        open spatial data directory. Data has also been provided by
                        the East Gippsland Shire Council.
                        
                        A second script has been prepared (CaseStudyTwo_DAPP) to
                        project the use-by year of policy options based upon 
                        principles from DAPP. 
                        
                        The script also uses a lookup table produced in
                        CaseTwo_AAPE_Lookup.py, and the results output from 
                        this script are used in the CaseTwo_PRIM.py script.
...
cases = cases[0:5000]    #Note [31:32] a good one to test
cases
...
#    prop_impact = prop_impact[0:1]     # For troubleshooting    
#    prop_impact
 
#    prop_impact['slr_adj'] = slr
#    prop_impact['slr_adj'] = 0
...
#    prop_impact.to_csv(Property_outfile_one) 
    ...
    fld_pt_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']== round(fld_lvl_pt_one,2), 'NO_OP_AAPE'])
    ppl_pt_one = fld_pt_one_lookup*(1+a_ppl)
    NO_PPL_PT_ONE.append(ppl_pt_one)
       
    fld_pt_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_two,2), 'NO_OP_AAPE'])
    ppl_pt_two = fld_pt_two_lookup*(1+a_ppl)
    NO_PPL_PT_TWO.append(ppl_pt_two)

    fld_pt_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_five,2), 'NO_OP_AAPE'])
    ppl_pt_five = fld_pt_five_lookup*(1+a_ppl)
    NO_PPL_PT_FIVE.append(ppl_pt_five)    
    
    fld_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_one,2), 'NO_OP_AAPE'])
    ppl_one = fld_one_lookup*(1+a_ppl)
    NO_PPL_ONE.append(ppl_one)    
    
    fld_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_two,2), 'NO_OP_AAPE'])
    ppl_two = fld_two_lookup*(1+a_ppl)
    NO_PPL_TWO.append(ppl_two)    
    
    fld_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_five,2), 'NO_OP_AAPE'])
    ppl_five = fld_five_lookup*(1+a_ppl)
    NO_PPL_FIVE.append(ppl_five)     
    
    fld_ten_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_ten,2), 'NO_OP_AAPE'])
    ppl_ten = fld_ten_lookup*(1+a_ppl)
    NO_PPL_TEN.append(ppl_ten)     
    
    fld_twenty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_twenty,2), 'NO_OP_AAPE'])
    ppl_twenty = fld_twenty_lookup*(1+a_ppl)
    NO_PPL_TWENTY.append(ppl_twenty) 

    fld_forty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_forty,2), 'NO_OP_AAPE'])
    ppl_forty = fld_forty_lookup*(1+a_ppl)
    NO_PPL_FORTY.append(ppl_forty)

    fld_sixty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_sixty,2), 'NO_OP_AAPE'])
    ppl_sixty = fld_sixty_lookup*(1+a_ppl)
    NO_PPL_SIXTY.append(ppl_sixty)

    fld_ninety_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_ninety,2), 'NO_OP_AAPE'])
...
#    print "-------------------------------------"     
################################################################################       
    #        pol_a_houses = sum(NO_HOUSES_ONE_POL_A)
    #        SUM_HOUSES_ONE_POL_A.append(pol_a_houses)
    #    
    #        AEP_one_impact_pol_a = sum(TOTAL_DAM_ONE_POL_A)
    #        AEP_ONE_IMP_POL_A.append(AEP_one_impact_pol_a)
    #        AEP_pt_one_impact_pol_a = sum(TOTAL_DAM_PT_ONE_POL_A)
    #        AEP_PT_ONE_IMP_POL_A.append(AEP_pt_one_impact_pol_a)
    #        AEP_pt_two_impact_pol_a = sum(TOTAL_DAM_PT_TWO_POL_A)
    #        AEP_PT_TWO_IMP_POL_A.append(AEP_pt_two_impact_pol_a)
    #        AEP_pt_five_impact_pol_a = sum(TOTAL_DAM_PT_FIVE_POL_A)
    #        AEP_PT_FIVE_IMP_POL_A.append(AEP_pt_five_impact_pol_a)              
    #        AEP_two_impact_pol_a = sum(TOTAL_DAM_TWO_POL_A)
    #        AEP_TWO_IMP_POL_A.append(AEP_two_impact_pol_a)
    #        AEP_five_impact_pol_a = sum(TOTAL_DAM_FIVE_POL_A)
    #        AEP_FIVE_IMP_POL_A.append(AEP_five_impact_pol_a)
    #        AEP_ten_impact_pol_a = sum(TOTAL_DAM_TEN_POL_A)
    #        AEP_TEN_IMP_POL_A.append(AEP_ten_impact_pol_a)
    #        AEP_twenty_impact_pol_a = sum(TOTAL_DAM_TWENTY_POL_A)
    #        AEP_TWENTY_IMP_POL_A.append(AEP_twenty_impact_pol_a)
...
#            TOTAL_DAM_ONE_POL_A = []   
#            TOTAL_DAM_PT_ONE_POL_A = []
#            TOTAL_DAM_PT_TWO_POL_A = []
#            TOTAL_DAM_PT_FIVE_POL_A = []
#            TOTAL_DAM_TWO_POL_A = []
#            TOTAL_DAM_FIVE_POL_A = []
#            TOTAL_DAM_TEN_POL_A = []
#            TOTAL_DAM_TEWNTY_POL_A = []
           
            #Append to a new dataframe
#        RDM_LSUB.append(l_sub)
#        RDM_APOLY.append(a_poly)
#        RDM_BPOLY.append(b_poly)
#        RDM_CPOLY.append(c_poly)
#        RDM_CSTAR.append(c_star)
#        RDM_TSTAR.append(t_star)
#        RDM_RSEA.append(r_sea)
#        RDM_DI.append(d_i)
#        #    RDM_SLR_PROJ.append(slr_proj)
#        RDM_RSTR.append(r_str)
#        RDM_RCON.append(r_con)
#        RDM_R.append(r)
#        RDM_TLEADA.append(t_lead_pol_a)
#        RDM_LCAPA.append(l_cap_pol_a)
#        RDM_MOPA.append(m_op_pol_a)
#        RDM_PLIFEA.append(p_life_pol_a)
#        YYYY.append(yr_yyyy)
#        CASE.append(case)
...
Cases_outfile_one = ("C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Case_Results_RDM.csv")
