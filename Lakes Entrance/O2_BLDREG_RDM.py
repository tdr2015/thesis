...
##############################################################     
##############################################################
    # Change property stock details in ZONE 1, 2, 3, 4
    # Create a subset of the proporty database with zone 1,2,3,4 only            
    len(prop_impact) 
...
#    sample2 = []
    ...
    prop_imp_sub['FloorTwo'] = FLRTWO
    prop_imp_sub['VulnModel'] = VM     
    prop_imp_sub.head()
    # Replace the value in the original dataset 
    for index, j in prop_imp_sub.iterrows():          
        ID = int(j['Arc_ID'])            
        NewLVL = prop_imp_sub.loc[prop_imp_sub['Arc_ID'] == ID, 'FloorTwo']
        NewVM = prop_imp_sub.loc[prop_imp_sub['Arc_ID'] == ID, 'VulnModel']
...
#    Check2 = ("C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Check_prop_impact.csv")
#    prop_impact.to_csv(Check2) 
...
#    prop_impact.to_csv(Property_outfile_one) 
    ...
    fld_pt_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_one,2), 'OP2_AAPE'])
    ppl_pt_one = fld_pt_one_lookup*(1+a_ppl)
    NO_PPL_PT_ONE.append(ppl_pt_one)
       
    fld_pt_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_two,2), 'OP2_AAPE'])
    ppl_pt_two = fld_pt_two_lookup*(1+a_ppl)
    NO_PPL_PT_TWO.append(ppl_pt_two)

    fld_pt_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_pt_five,2), 'OP2_AAPE'])
    ppl_pt_five = fld_pt_five_lookup*(1+a_ppl)
    NO_PPL_PT_FIVE.append(ppl_pt_five)    
    
    fld_one_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_one,2), 'OP2_AAPE'])
    ppl_one = fld_one_lookup*(1+a_ppl)
    NO_PPL_ONE.append(ppl_one)    
    
    fld_two_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_two,2), 'OP2_AAPE'])
    ppl_two = fld_two_lookup*(1+a_ppl)
    NO_PPL_TWO.append(ppl_two)    
    
    fld_five_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_five,2), 'OP2_AAPE'])
    ppl_five = fld_five_lookup*(1+a_ppl)
    NO_PPL_FIVE.append(ppl_five)     
    
    fld_ten_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_ten,2), 'OP2_AAPE'])
    ppl_ten = fld_ten_lookup*(1+a_ppl)
    NO_PPL_TEN.append(ppl_ten)     
    
    fld_twenty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_twenty,2), 'OP2_AAPE'])
    ppl_twenty = fld_twenty_lookup*(1+a_ppl)
    NO_PPL_TWENTY.append(ppl_twenty) 
       
    fld_forty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_forty,2), 'OP2_AAPE'])
    ppl_forty = fld_forty_lookup*(1+a_ppl)
    NO_PPL_FORTY.append(ppl_forty)

    fld_sixty_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_sixty,2), 'OP2_AAPE'])
    ppl_sixty = fld_sixty_lookup*(1+a_ppl)
    NO_PPL_SIXTY.append(ppl_sixty)

    fld_ninety_lookup = float(aape_lookup.loc[aape_lookup['Flood_level']==round(fld_lvl_ninety,2), 'OP2_AAPE'])
...
Cases_outfile_one = ("C:\\Users\\tdramm\\Desktop\\GIS_LE\\zzCODE_TESTING\\"+date_string+"_Case_Results_RDM_OPTION2_BLDREG.csv")
